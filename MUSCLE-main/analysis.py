...
def main():
    ...
    # 加载刚才保存的最优权重
    try:
        model.load_state_dict(torch.load('best_muscle_model.pth', map_location=device))
        print("Successfully loaded 'best_muscle_model.pth'!")
    except FileNotFoundError:
        print("Error: 'best_muscle_model.pth' not found! Please run main.py first to generate it.")
        return
    ...
    with torch.no_grad():
        forward_res = model(mashup_sim_matrix, api_sim_matrix, interaction_adj, mashup_des_emb, api_des_emb)
        final_mashup = forward_res[0]
        final_api = forward_res[1]
        final_api_embeddings = final_api.cpu().numpy()
    ...
    # ================= 排版与渲染 =================
    fig, (ax_top, ax_bottom) = plt.subplots(2, 1, figsize=(4, 5.5), gridspec_kw={'height_ratios': [2.5, 1]})
    ...
    ax_top.set_title('MUSCLE', fontsize=15, fontweight='bold', pad=10)
    ...
    # 动态缩放：确保最高密度峰值适配 1.0 的 Y轴高度 (替代写死的 *3)
    ...
    plt.savefig("visual_analysis_slim.png", dpi=600, bbox_inches='tight')
    print("-> Plot saved as 'visual_analysis_slim.png'!")
